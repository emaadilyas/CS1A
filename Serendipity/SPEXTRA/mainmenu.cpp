		...
		switch (choice)
		{
			case '1':
				if (numRecords == 0)
				{
					...
					cout << "Enter a record in 'Add Book' before trying to search." << endl;
					cout << "Press ENTER to continue ..." << endl;
					cin.get();
				}
				else
			...
			case '3':
				...
				{
					editBook(bookTitle, isbn, author, publisher, dateAdded, qtyOnHand, wholesale, retail, numRecords);
				}
				break;
			case '4':
				deleteBook();
...
void editBook(string bookTitle[], string isbn[], string author[], string publisher[], string dateAdded[], int qtyOnHand[], double wholesale[], double retail[], size_t &numRecords)
{
	size_t recordChoice;
	cout << CLEAR_SCREEN;
	cout << "Enter the record number you would like to edit (1-" << numRecords << ").";
	cin >> recordChoice; // reading which record they want to change

	size_t index = recordChoice - 1; // just converting the int to a recordChoice

	if (recordChoice < 1 || recordChoice > numRecords)
	{
		cout << "Invalid record, please try again." << endl;
		cout << "Returning to Inventory Menu ..." << endl;
		cout << "Press ENTER to continue ..." << endl;
		cin.get();
		return;
	...
	char choice;
	string pendingBookTitle = bookTitle[index];
	string pendingISBN      = isbn[index];
	string pendingAuthor    = author[index];
	string pendingPublisher = publisher[index];
	string pendingDateAdded = dateAdded[index];
	int pendingQtyOnHand 	= qtyOnHand[index];
	double pendingWholesale = wholesale[index];
	double pendingRetail    = retail[index];
	bool enteredData = false;
	bool saveData = false;
	...
	do
	{
		...
		string title = "Serendipity Booksellers";
		string title2 = "Add Book";
		string title3 = "Database Size: " + to_string(DBSIZE) + " Editing Record #" + to_string(index);
		string opt1 = " <1> Enter Book Title               >  --" + pendingBookTitle;
		string opt2 = " <2> Enter Book ISBN                >  --" + pendingISBN;
		string opt3 = " <3> Enter Author                   >  --" + pendingAuthor;
		string opt4 = " <4> Enter Publisher                >  --" + pendingPublisher;
		...
		switch (choice)
		{
			...
			case '9': // save book
			{
				...
				{
					// saving all of the temp variables to actual ones
					bookTitle[index] = pendingBookTitle;
	    	        isbn[index]      = pendingISBN;
	        	    author[index]    = pendingAuthor;
	            	publisher[index] = pendingPublisher;
	            	dateAdded[index] = pendingDateAdded;
	            	qtyOnHand[index] = pendingQtyOnHand;
	        	    wholesale[index] = pendingWholesale;
   		            retail[index]    = pendingRetail;
					...
					cout << endl;
					cout << "The record " << index << " has been updated..." << endl;
					cout << "Returning to Inventory Menu." << endl;
					cout << "Press ENTER to continue ..." << endl;
					cin.get();
					saveData = true;
